...
* The @gather@-step in @PipesPipeline@ has the transformation step internal to it and thus, not two pipes when PipeFunction provided
* @GatherPipe@ no longer locks on looping and is semantically sound for depth-first traversing
* Added @Pipe.enablePath(boolean)@ method to support the turning on/off of path calculations
...
* Fixed an indexing issues in @ExhaustMergePipe@ that made itself apparent when two split/merges happened in the same pipeline
* Added @TreePipe@ to return a tree representation of the traversal
